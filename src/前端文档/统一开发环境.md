...
> 很多时候，同一个项目换台电脑运行就可能出现运行报错、打包失败的情况，这可能是运行环境不同造成的，所以团队内统一开发环境很有必要。
...
- 包管理器及第三方包
- IDE、IDE扩展
- 命令行环境（Windows、类Unix 等）

解决运行环境不一致的问题也就是统一上述的关键环境

## Node

> 项目或引用的第三方包中，很有可能用到了特定版本范围的 node 接口，为了项目正常运行，就必须正确限制团队成员所使用的 node 版本
...
> 需要注意切换一个新安装的版本时，需要重新进行 corepack配置、全局包安装等操作
...
npm 的版本配置也是如此，配置规则与项目依赖（dependencies）保持一致，支持 `^`、`~`、`> < =`、`*`、`||` 等符号

仅作如上配置时，engines 字段只起建议的作用，只有在 `.npmrc` 文件中配置了 `engine-strict = true` 时，才会强制要求使用 engines 字段配置的版本。

## 包管理器

> 不同的包管理器会生成不同的 lock 文件，而 lock 文件决定了本地的第三方包版本，为了各成员依赖的代码一致，需要要求统一包管理器。
>
...
例如在类 Unix 系统（Mac、Linux 等）中常用的删除命令 `rm -rf`，在 Windows 系统中就无法运行，为了解决这个问题，应该使用兼容性更好的第三方包来实现这些功能。
