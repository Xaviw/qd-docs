...
VSCode 编辑器自带 TypeScript 语言支持，官方会在编辑器升级时一并升级 TS 版本

为了团队成员能够使用统一的 TS 版本，一般需要在项目中将 typescript 安装为开发时依赖，并通过工作区设置编辑器使用依赖安装的 TS 版本，而不是编辑器自带版本
...
在使用包管理器安装依赖后，会生成 `lock` 文件，之后执行安装命令时便会使用 `lock` 文件中锁定的版本（也会根据 `package.json` 的修改进行更新）

所以 `lock` 文件应该提交到仓库中，团队成员始终遵照 `lock` 文件中的版本进行安装，避免随意重新生成 `lock` 文件（例如删除 `lock` 文件后重新安装或使用 `--force` 命令）。在部署流程中（例如 docker 部署）也应该同时上传 `lock` 文件，保证其他环境依赖版本与开发环境一致
