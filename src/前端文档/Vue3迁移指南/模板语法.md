...
Vue2 中可以通过 `this.$listeners` 访问传递给组件的事件监听器，Vue3 中移除了 `$listeners`，现在事件只是 `$attrs` 中以 `on` 开头的属性

Vue2 与 Vue3 均可以设置 `inheritAttrs` 选项为 `false`，然后手动将属性和事件应用到根元素之外的元素上
...
在 Vue2 中，`class` 和 `style` 并不包含在 `$attrs` 中，始终只能绑定到组件根元素

Vue3 改变了这一点，现在 `$attrs` 包含所有的属性，可以将 `class` 和 `style` 随其他属性一起绑定到非根元素上
...
随着 [KeyboardEvent.keyCode](https://developer.mozilla.org/zh-CN/docs/Web/API/KeyboardEvent/keyCode) 被废弃，Vue3 改为直接使用 `kebab-cased` (短横线) 按键名称来作为修饰符
