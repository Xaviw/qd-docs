...
在 Vue2 中除了封装组件之外，复用逻辑基本只有通过 mixin 的方式，但使用 mixin 后逻辑难以溯源，如果存在嵌套则更是混乱
...
创建组合式函数和创建普通工具函数的思路类似，区别在于需要结合 Vue 的响应式原理，搞清楚哪些数据需要响应，哪些数据不需要响应，以及数据的响应会对外部带来哪些影响等

可以从 [VueUse](https://vueuse.pages.dev/functions.html) 库中找到一些抽取组合式函数的灵感，熟悉库中已经提供的组合式函数也可以减少开发复杂度

类似的库还有 [VueHooksPlus](https://inhiblabcore.github.io/docs/hooks/guide/)
