...
在 Vue2 中除了封装组件之外，复用逻辑基本只有通过 mixin 的方式，但使用 mixin 后逻辑难以溯源，如果存在嵌套更是难以理清

Vue3 改用组合式 API 后，响应式 API 可以脱离组件使用，这使得 Vue3 拥有了函数式编程的优点。官方称利用响应式 API 实现的函数为组合式函数（Composables），类似于 React 中的 `Hooks` 概念，按照规范组合式函数命名应该以 `use` 开头，通常会在项目中创建一个 `composables` 目录用于存放全局的组合式函数（与 `utils` 目录同级）

创建组合式函数和创建普通工具函数的思路类似，区别在于需要结合 Vue 的响应式原理，搞清楚哪些数据需要响应，哪些数据不需要响应，以及数据的响应式会对外部带来哪些影响等

例如表格操作，通常有操作一条和操作多条的需求，例如删除、启用、禁用等，他们的操作流程都是类似的，只有调用 API 不同，完全可以封装成组合式函数

<IframeContainer title="组合式函数例子" src="https://play.vuejs.org/#" />
